...
function banner(){
	...
	printf "        ▄▄▄        ██████ ▄▄▄█████▓ ██▀███   ▄▄▄        \n"
    	...
    	printf "        ░   ▒   ░  ░  ░    ░        ░░   ░   ░   ▒      \n"
    	printf "            ░  ░      ░              ░           ░  ░   \n"
	printf " ${astra_version}                                 by @six2dez${reset}\n"
}
...
function google_dorks(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$GOOGLE_DORKS" = true ] && [ "$OSINT" = true ]; then
		...
		if [ "$GOOGLE_DORKS" = false ] || [ "$OSINT" = false ]; then
			printf "\n${yellow} ${FUNCNAME[0]} skipped in this mode or defined in astra.cfg ${reset}\n"
		else
...
function sub_gpt(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$SUBGPT" = true ] && [ -s "$SUBGPT_COOKIE" ]; then
		...
		if [ "$SUBGPT" = false ]; then
			...
			printf "\n${yellow} ${FUNCNAME[0]} SUBGPT_COOKIE not defined on config file (astra.cfg by default) ${reset}\n"
		else
			printf "${yellow} ${FUNCNAME[0]} is already processed, to force executing ${FUNCNAME[0]} delete\n    $called_fn_dir/.${FUNCNAME[0]} ${reset}\n\n"
		fi
	fi
...
function sub_recursive_brute(){
	if { [ ! -f "$called_fn_dir/.${FUNCNAME[0]}" ] || [ "$DIFF" = true ]; } && [ "$SUB_RECURSIVE_BRUTE" = true ] && [ -s "subdomains/subdomains.txt" ]; then
		...
		if [[ $(cat subdomains/subdomains.txt | wc -l) -le $DEEP_LIMIT ]] ; then
			...
			end_subfunc "skipped in this mode or defined in astra.cfg" ${FUNCNAME[0]}
		fi
		if [ "$INSCOPE" = true ]; then
...
function sendToNotify {
	if [[ -z "$1" ]]; then
		...
		if [[ -n "$slack_channel" ]] && [[ -n "$slack_auth" ]]; then
			...
			curl -F file=@${1} -F "initial_comment=astra zip file" -F channels=${slack_channel} -H "Authorization: Bearer ${slack_auth}" https://slack.com/api/files.upload 2>>"$LOGFILE" &>/dev/null
		fi
	fi
}
...
function ipcidr_target(){
	...
	if [[ $1 =~ ^$IP_CIDR_REGEX ]]; then
		echo $1 | mapcidr -silent | anew -q target_astra_ipcidr.txt
		if [ -s "./target_astra_ipcidr.txt" ]; then 
			[ "$REVERSE_IP" = true ] && cat ./target_astra_ipcidr.txt | hakip2host | cut -d' ' -f 3 | unfurl -u domains 2>/dev/null | sed 's/\.$//' | anew -q ./target_astra_ipcidr.txt
			if [[ $(cat ./target_astra_ipcidr.txt | wc -l) -eq 1 ]]; then
				domain=$(cat ./target_astra_ipcidr.txt)
			elif [[ $(cat ./target_astra_ipcidr.txt | wc -l) -gt 1 ]]; then
				unset domain
				list=${PWD}/target_astra_ipcidr.txt
			fi
		fi
		if [ -n "$2" ]; then
			cat $list | anew -q $2
...
function help(){
	...
	printf "   -f config_file    Alternate astra.cfg file\n"
...
case $opt_mode in
        ...
        'w')
			if [ -n "$list" ]; then
				...
				printf "\n\n${bred} Web mode needs a website list file as target (./astra.sh -l target.txt -w) ${reset}\n\n"
				exit
			fi
			webs_menu
			exit
