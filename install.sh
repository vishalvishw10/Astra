...
function banner_web(){
    ...
    printf "            ░  ░      ░              ░           ░  ░   \n"
        ...
        printf " ${Astra_version}                                         by @Vishal\n"
}


install_webserver(){
    printf "${bblue} Running: Installing web astra ${reset}\n\n"
...
banner_web
printf "\n${bgreen} astra installer/updater script ${reset}\n\n"
...
while true; do
    ...
    if $rftw_installed; then
        printf "${bblue} 1. Install/Update astra (without Web Interface)${reset}\n\n"
        printf "${bblue} 2. Install/Update astra + Install Web Interface${reset} ${yellow}(User Interaction needed!)${reset}\n\n"
        ...
        printf "${bblue} 1. Install/Update astra${reset}\n\n"
        printf "${bblue} 2. Install/Update astra + Install Web Interface${reset} ${yellow}(User Interaction needed!)${reset}\n\n"
        printf "${bred} 3. Can't install Web Interface without astra${reset}\n\n"
        printf "${bblue} 4. Exit${reset}\n\n"
        printf "${bgreen}#######################################################################${reset}\n\n"
        read -p "$(echo -e ${bblue} "Insert option: "${reset})" option
        printf "\n${bgreen}#######################################################################${reset}\n\n"
...
if [[ $(id -u | grep -o '^0$') == "0" ]]; then
    ...
    if sudo -n false 2>/dev/null; then
        ...
        printf "${bred} echo \"${USERNAME}  ALL=(ALL:ALL) NOPASSWD: ALL\" > /etc/sudoers.d/astra${reset}\n\n"
    fi
    SUDO="sudo"
fi
...
printf "${bblue} Running: Looking for new astra version${reset}\n\n"
...
then
    ...
    if git status --porcelain | grep -q 'astra.cfg$'; then
        mv astra.cfg astra.cfg_bck
        printf "${yellow} astra.cfg has been backed up in astra.cfg_bck${reset}\n\n"
    fi
    eval git reset --hard $DEBUG_STD
    eval git pull $DEBUG_STD
    printf "${bgreen} Updated! Running new installer version...${reset}\n\n"
else
    printf "${bgreen} astra is already up to date!${reset}\n\n"
fi
...
EOF
    ...
    printf "${byellow} Golang will not be configured according to the user's prefereneces (astra.cfg install_golang var)${reset}\n";
fi
...
wget -q -O - https://raw.githubusercontent.com/six2dez/resolvers_astra/main/resolvers_trusted.txt > ${resolvers_trusted}
...
if [ "$generate_resolvers" = true ]; then
	if [ ! -s "$resolvers" ] || [[ $(find "$resolvers" -mtime +1 -print) ]] ; then
        ...
        [ ! -s "$resolvers_trusted" ] && wget -q -O - https://raw.githubusercontent.com/six2dez/resolvers_astra/main/resolvers_trusted.txt > ${resolvers_trusted}
